...
def data_transformation(image, left_landmark, right_landmark):
    x = random_crop_resize(image, left_landmark, right_landmark)
    x = color_distortion(x)
    x, angle = image_rotation(x)

    return x, angle

if __name__ == '__main__':

    jpeg_file = '/home/lingyu/MPIIFaceGaze_normalized/MPIIFaceGaze_normalizad/mix_dataset/29991.jpg'
    #left_landmarks = np.load('/home/lingyu/MPIIFaceGaze_normalized/MPIIFaceGaze_normalizad/p02/left_landmarks.npy')
    #right_landmarks = np.load('/home/lingyu/MPIIFaceGaze_normalized/MPIIFaceGaze_normalizad/p02/right_landmarks.npy')
    img = io.imread(jpeg_file)/255.
    #img = random_crop_resize(img, left_landmarks[img_number], right_landmarks[img_number])
    #tar = '/home/lingyu/crop_image/' + str(img_number) + '_'+str(i)+'.jpg'
    #plt.imsave(tar, img)
    plt.imshow(img)
    plt.show()
    a = 5